				...
				struct station_info *sinfo)
{
	...
	)
	{
		...
		sinfo->filled |= (1<<6);//STATION_INFO_SIGNAL;
		sinfo->signal = translate_percentage_to_dbm(padapter->recvpriv.signal_strength);

		sinfo->filled |= (1<<7);//STATION_INFO_TX_BITRATE;
		sinfo->txrate.legacy = rtw_get_cur_max_rate(padapter);

		sinfo->filled |= (1<<8);//STATION_INFO_RX_PACKETS;
		sinfo->rx_packets = sta_rx_data_pkts(psta);

		sinfo->filled |= (1<<9);//STATION_INFO_TX_PACKETS;
...
void rtw_cfg80211_indicate_sta_assoc(_adapter *padapter, u8 *pmgmt_frame, uint frame_len)
{
...
#if defined(RTW_USE_CFG80211_STA_EVENT) || defined(COMPAT_KERNEL_RELEASE)
	{
		...
		sinfo.filled = 0;
		sinfo.filled = (1<<17);//STATION_INFO_ASSOC_REQ_IES;
