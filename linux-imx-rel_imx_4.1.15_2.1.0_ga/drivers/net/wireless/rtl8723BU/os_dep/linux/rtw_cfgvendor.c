	...
	struct net_device *dev, void  *data, int len, wl_vendor_event_t event)
{
	...
	while (total > 0) {
		...
		/* Alloc the SKB for vendor_event */
		skb = rtw_cfg80211_vendor_event_alloc(wiphy, NULL, malloc_len, event, kflags);
		if (!skb) {
...
void wl_cfgvendor_rtt_evt(void *ctx, void *rtt_data)
{
	...
	kflags = in_atomic() ? GFP_ATOMIC : GFP_KERNEL;
	/* Alloc the SKB for vendor_event */
	skb = rtw_cfg80211_vendor_event_alloc(wiphy, NULL, tot_len, GOOGLE_RTT_COMPLETE_EVENT, kflags);
	if (!skb) {
		WL_ERR(("skb alloc failed"));
		goto exit;
	}
